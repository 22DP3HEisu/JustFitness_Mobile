...
const calorieItems = [{
  labelKey: "ui.meals",
  key: 'caloriesFromMeals',
  icon: 'restaurant-outline',
  iconSet: 'ion'
}, {
  labelKey: "ui.workouts",
  key: 'caloriesFromWorkouts',
  icon: 'dumbbell',
  iconSet: 'material'
}, {
  labelKey: "ui.steps",
...
const formatShortDateLabel = date => date.toLocaleDateString('en', {
  month: 'short',
  day: 'numeric',
...
const normalizeDashboard = payload => {
  ...
  const entries = (nextDashboard.weight.entries || []).map(entry => ({
    ...entry,
    date: entry?.date || entry?.created_at,
    weight: Number(entry?.weight)
  })).filter(entry => Number.isFinite(entry.weight)).sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  return {
    ...
    weight: {
      ...
      latest,
      comparison,
      change: latest != null && comparison != null ? Number((latest - comparison).toFixed(1)) : nextDashboard.weight.change ?? null,
...
const WeightChart = ({
  ...
  if (!chartEntries.length) {
    return <View style={[styles.chart, styles.emptyChart, {
      ...
      height: chartHeight + 42
        ...
        <Text style={styles.emptyChartText}>{i18n.t("ui.no_weight_logs")}</Text>
      </View>;
  }
  ...
  return <View style={[styles.chart, {
    ...
    height: chartHeight + 42
      ...
      {[0.18, 0.39, 0.59, 0.79].map(position => <View key={position} style={[styles.verticalGridLine, {
      ...
      {points.map((point, index) => <View key={`point-${index}`} style={[styles.chartPoint, {
      left: point.x - 5,
      top: point.y - 5
    }]} />)}
      ...
      width: plotWidth
        ...
        {Array.from({
        length: 28
      }).map((_, index) => <View key={index} style={[styles.dot, index % 7 === 0 && styles.bigDot]} />)}
      </View>
...
const DashboardScreen = () => {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      const {
        data
      } = await authFetch(`/api/dashboard?weightPeriod=${encodeURIComponent(weightPeriod)}`);
      if (!data.success) {
        setError(data.message || i18n.t("ui.failed_to_fetch_dashboard_data"));
        return;
      }
      setDashboard(normalizeDashboard(data.data));
    } catch (err) {
      console.error('Error fetching dashboard:', err);
      setError(err.message || i18n.t("ui.failed_to_load_dashboard"));
    } finally {
      setIsLoading(false);
    }
  ...
  return <View style={styles.container}>
      ...
      <LinearGradient colors={['rgba(58, 78, 72, 0.55)', 'rgba(58, 78, 72, 0.88)', 'rgba(58, 78, 72, 1)']} style={styles.overlay}>
        {isLoading ? <View style={styles.centerState}>
            <ActivityIndicator size="large" color={COLORS.yellow} />
            <Text style={styles.stateText}>{i18n.t("ui.loading_dashboard")}</Text>
            ...
            <View style={styles.caloriesCard}>
              <View style={styles.calorieRingWrap}>
                <SegmentedRing progress={ringProgress} size={132} thickness={8}>
                  ...
                  <Text style={styles.ringLabel}>{i18n.t("ui.remaining")}</Text>
                </SegmentedRing>
              </View>

              <View style={styles.goalPanel}>
                <View style={styles.goalHeader}>
                  <Text style={styles.cardTitle}>{i18n.t("ui.calories_goal")}</Text>
                  <Text selectable style={styles.goalValue}>{Math.round(calorieGoal)}</Text>
                </View>
                <Text style={styles.goalFormula}>{i18n.t("ui.remaining_goal_meals_exercise")}</Text>

                <View style={styles.goalRows}>
                  {calorieItems.map(item => {
                const value = item.key === 'steps' ? Math.round(stepsToday) : Math.round(asNumber(dashboard.nutrition[item.key]));
                const label = i18n.t(item.labelKey);
                return <View style={styles.goalRow} key={item.key}>
            ...
            <View style={styles.twoColumn}>
              <View style={styles.statCard}>
                <View style={styles.statTitleRow}>
                  ...
                  <Text style={styles.statName}>{i18n.t("ui.workouts")}</Text>
                </View>
            ...
            <View style={styles.macrosCard}>
              ...
              <View style={styles.macroPanel}>
                <View style={styles.macroHeader}>
                  <Text style={styles.cardTitle}>{i18n.t("ui.macros")}</Text>
                  <Text style={styles.todayText}>{i18n.t("ui.today")}</Text>
                </View>
                <View style={styles.macroTotals}>
                  <Text style={styles.macroSmallHeader}>{i18n.t("ui.total")}</Text>
                  <Text style={[styles.macroSmallHeader, {
                color: COLORS.yellow
              }]}>{i18n.t("ui.goal_2")}</Text>
                </View>
                {macroRows.map(item => <View style={styles.macroRow} key={item.name}>
                    <View style={[styles.macroSwatch, {
            ...
            <View style={styles.weightCard}>
              <View style={styles.periodTabs}>
                {weightPeriods.map(period => <TouchableOpacity key={period.key} style={[styles.periodTab, weightPeriod === period.key && styles.periodTabActive]} onPress={() => setWeightPeriod(period.key)}>
                    <Text style={[styles.periodText, weightPeriod === period.key && styles.periodActive]}>
                      {i18n.t(period.labelKey)}
                    </Text>
                  </TouchableOpacity>)}
              </View>
              ...
              <View style={styles.weightHeader}>
                <View style={styles.weightTitleRow}>
                  ...
                  <Text style={styles.cardTitle}>{i18n.t("ui.weight")}</Text>
                </View>
              ...
              <View style={styles.weightStats}>
                <View style={styles.weightStat}>
                  ...
                  <Text style={styles.weightLabel}>{i18n.t("ui.weight")}</Text>
                </View>
                <View style={styles.weightStat}>
                  <Text selectable style={styles.weightValue}>
                    {comparisonWeight == null ? '--' : `${formatNumber(comparisonWeight, 1)} ${weightUnit}`}
                  </Text>
                  <Text style={styles.weightLabel}>{i18n.t(`ui.weight_${weightPeriod}_ago`)}</Text>
                </View>
                <View style={styles.weightStat}>
                  <Text selectable style={styles.weightValue}>
                    {dashboard.weight.change == null ? '--' : `${formatNumber(dashboard.weight.change, 1)} ${weightUnit}`}
                  </Text>
                  <Text style={styles.weightLabel}>{i18n.t("ui.weight_changes")}</Text>
                </View>
                <View style={styles.weightStat}>
                  <Text selectable style={styles.weightValue}>
                    {dashboard.weight.goal == null ? '--' : `${formatNumber(dashboard.weight.goal, 0)} ${weightUnit}`}
                  </Text>
                  <Text style={styles.weightLabel}>{i18n.t("ui.goal_2")}</Text>
                </View>
              </View>
            </View>
          </ScrollView>}
