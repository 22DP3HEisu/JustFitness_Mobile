...
const normalizeSessionLogs = (exerciseLogs) => (
...
const WorkoutSessionScreen = () => {
  ...
  const persistLocalDraft = () => {
    ...
    return AsyncStorage.setItem(
      getDraftStorageKey(session.id),
      JSON.stringify({ exerciseLogs, setDrafts })
    )
  }

  const minimizeSession = ({ persistDraft = true } = {}) => {
    if (!persistDraft) {
      dismissSessionModal()
      return
    }
  ...
  const fetchOngoingWorkout = async () => {
    ...
    try {
      ...
      const { data } = await authFetch('/api/workouts/session/ongoing')
      ...
      Alert.alert('Error', error.message || 'Failed to load workout session')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const addSet = async (exerciseLog) => {
    ...
    setExerciseLogs(prev => prev.map(item => (
      item.localId === exerciseLog.localId
  ...
  const removeExercise = async (exerciseLog) => {
    Alert.alert('Remove exercise?', `Remove ${exerciseLog.exercise_name} and its sets from this session?`, [
  ...
  const getCompletionExercisesPayload = () => (
  ...
  const completeWorkout = async () => {
    ...
    Alert.alert('Finish workout?', 'Complete this active workout session?', [
      { text: 'Keep training', style: 'cancel' },
      {
        text: 'Finish',
        onPress: async () => {
          try {
            const { data } = await authFetch(`/api/workouts/session/${session.id}/complete`, {
              method: 'POST',
              body: JSON.stringify({
            ...
            if (data.success) {
              ...
              Alert.alert('Workout complete', 'Nice work. Your session has been saved.', [
                { text: 'OK', onPress: () => minimizeSession({ persistDraft: false }) },
              ])
            } else {
              Alert.alert('Error', data.message || 'Failed to complete workout')
            }
          } catch (error) {
  ...
  const cancelWorkout = async () => {
    ...
    Alert.alert('Cancel workout?', 'This will discard the active workout session.', [
      { text: 'Keep training', style: 'cancel' },
      {
        text: 'Cancel workout',
        style: 'destructive',
        onPress: async () => {
          try {
            const { data } = await authFetch(`/api/workouts/session/${session.id}`, {
              method: 'DELETE',
            })

            if (data.success) {
              await clearLocalDraft(session.id)
              minimizeSession({ persistDraft: false })
            } else {
              Alert.alert('Error', data.message || 'Failed to cancel workout')
            }
          } catch (error) {
  ...
  const renderSet = (exerciseLog, setLog, index) => {
    const draft = setDrafts[setLog.localId] || { reps: '', weight: '' }
    const totalSets = exerciseLog.sets?.length || 0

    return (
      <View key={setLog.localId} style={styles.setRow}>
        ...
        <TextInput
          ...
          value={draft.reps}
          onChangeText={(value) => updateDraft(setLog.localId, 'reps', value)}
          placeholder="0"
        ...
        <TextInput
          ...
          value={draft.weight}
          onChangeText={(value) => updateDraft(setLog.localId, 'weight', value)}
          placeholder="-"
          placeholderTextColor="rgba(255,255,255,0.3)"
        />

        <TouchableOpacity
          onPress={() => removeSet(setLog, totalSets)}
          style={[styles.setRemoveButton, totalSets <= 1 && styles.disabledButton]}
          disabled={totalSets <= 1 || isMutating}
        >
  ...
  const renderExerciseLog = (exerciseLog, index) => (
    <View key={exerciseLog.localId} style={styles.exerciseCard}>
      <View style={styles.exerciseHeader}>
        ...
        <View style={styles.exerciseInfo}>
          ...
          <Text style={styles.exerciseMeta}>{exerciseLog.sets?.length || 0} sets</Text>
        </View>
        <TouchableOpacity
          onPress={() => removeExercise(exerciseLog)}
      ...
      <View style={styles.container}>
        ...
        <LinearGradient colors={['#324640', '#263934']} style={styles.overlay}>
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Loading active workout...</Text>
          </View>
        </LinearGradient>
      </View>
    )
  ...
  if (!workout) {
    return (
      <View style={styles.container}>
        ...
        <LinearGradient colors={['#324640', '#263934']} style={styles.overlay}>
          <View style={styles.loadingContainer}>
            <Text style={styles.loadingText}>No active workout found</Text>
        ...
        >
          <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            <View style={styles.hero}>
              ...
              {workout.description ? (
                <Text selectable style={styles.workoutDescription} numberOfLines={2}>{workout.description}</Text>
              ) : null}
            </View>
            ...
            <View style={styles.sectionHeader}>
              <View>
                ...
                <Text style={styles.sectionMeta}>Changes here only affect this workout session</Text>
              </View>
