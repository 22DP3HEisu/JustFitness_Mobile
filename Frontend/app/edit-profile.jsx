...
const EditProfileScreen = () => {
  ...
  const genderOptions = [
    { label: 'Izvelieties dzimumu', value: '' },
    { label: 'Virietis', value: 'Male' },
    { label: 'Sieviete', value: 'Female' },
    { label: 'Cits', value: 'Other' },
  ]

  const selectedGenderLabel = genderOptions.find((option) => option.value === gender)?.label || genderOptions[0].label
  const selectedHeightUnitLabel = heightUnit === 'in' ? 'ft / in' : 'cm'
  ...
  const formatHeight = () => {
    if (!height) return 'Izvelieties augumu'
    if (heightUnit === 'in') {
      const { feet, inches } = splitInches(height)
  ...
  const formatDate = (date) => {
    if (!date) return 'Izvelieties dzimsanas dienu'
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert('Veiksmigi', 'Profila dati atjauninati')
        router.back()
      } else {
        Alert.alert('Kluda', data.message || 'Neizdevas atjauninat profila datus')
      }
    } catch (error) {
      ...
      >
        ...
        >
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Dzimsanas diena</Text>
          ...
          <View style={styles.inputGroup}>
            <View style={styles.labelRow}>
              <Text style={styles.label}>Augums</Text>
