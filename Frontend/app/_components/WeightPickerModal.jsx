...
const splitWeight = (weight, fallback) => {
  ...
  if (!Number.isFinite(parsed)) {
    return { whole: fallback, decimal: 0 }
  }
...
const WeightPickerModal = ({
  ...
  const wholeOptions = useMemo(() => {
    const range = isPounds
      ? buildRange(66, 550, draftWhole)
      : buildRange(30, 250, draftWhole)
    return range
  }, [draftWhole, isPounds])
      ...
      <Pressable style={styles.backdrop} onPress={onClose}>
        <Pressable style={styles.sheet}>
          ...
          <View style={styles.pickerRow}>
            <Picker
              selectedValue={draftWhole}
              onValueChange={setDraftWhole}
              style={styles.picker}
              itemStyle={styles.pickerItem}
            >
              {wholeOptions.map((value) => (
                <Picker.Item key={value} label={String(value)} value={value} />
              ))}
            </Picker>
            <Picker
              selectedValue={draftDecimal}
              onValueChange={setDraftDecimal}
