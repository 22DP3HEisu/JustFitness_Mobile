...
const EditAccountScreen = () => {
  ...
  const handleSaveAccount = async () => {
    // Validēē formu
    if (!name.trim()) {
      Alert.alert('Kļūda', 'Lūdzu ievadiet vārdu')
      return
    }
    if (!email.trim() || !email.includes('@')) {
      Alert.alert('Kļūda', 'Lūdzu ievadiet derīgu e-pasta adresi')
      return
    }

    setIsSaving(true)
    try {
      const { response, data } = await authFetch('/api/user', {
        method: 'PUT',
        body: JSON.stringify({
          name: name.trim(),
          email: email.trim()
      ...
      if (response.ok) {
        ...
        Alert.alert('Veiksmīgi', 'Konts atjaunināts')
      } else {
        Alert.alert('Kļūda', data.message || 'Neizdevās atjaunināt kontu')
      }
    } catch (error) {
      ...
      >
        ...
        >
          ...
          <View style={styles.section}>
            ...
            >
              {isSaving ? (
                ...
                <>
                  <MaterialIcons name="save" size={20} color="#2C3E50" style={{ marginRight: 8 }} />
                  <Text style={styles.actionButtonText}>Saglabāt konta datus</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>Jūs varat rediģēt savu vārdu un e-pasta adresi</Text>
          </View>
        </ScrollView>
      </LinearGradient>
    </SafeAreaView>
