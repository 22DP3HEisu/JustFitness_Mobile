...
const formatWeight = (weight, unit) => {
  if (weight == null || weight === '') return i18n.t("ui.izvelieties_svaru");
  return `${Number(weight).toFixed(1)} ${unit === 'lb' || unit === 'lbs' ? 'lb' : 'kg'}`;
};
const formatUpdatedAt = dateString => {
  if (!dateString) return i18n.t("ui.nav_saglabata_svara");
...
const EditWeightScreen = () => {
  ...
  const loadWeightData = async () => {
    ...
    try {
      const {
        response,
        data
      } = await authFetch('/api/user/weight');
  ...
  const handleSave = async () => {
    if (!weight) {
      Alert.alert(i18n.t("ui.kluda"), i18n.t("ui.ludzu_izvelieties_svaru"));
      return;
    }
    setIsSaving(true);
    try {
      ...
      if (response.ok) {
        Alert.alert(i18n.t("ui.veiksmigi"), i18n.t("ui.svars_saglabats"));
        setUpdatedAt(data.data.created_at);
        router.back();
      } else {
        Alert.alert(i18n.t("ui.kluda"), data.message || i18n.t("ui.neizdevas_saglabat_svaru"));
      }
    } catch (error) {
      ...
      <LinearGradient colors={['rgba(58, 78, 72, 0.4)', 'rgba(58, 78, 72, 0.8)', 'rgba(58, 78, 72, 0.95)']} style={styles.overlay}>
        ...
        <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          <View style={styles.inputGroup}>
            <View style={styles.labelRow}>
              <Text style={styles.label}>{i18n.t("ui.pasreizejais_svars")}</Text>
              <Text style={styles.unitText}>{weightUnit === 'lb' || weightUnit === 'lbs' ? 'lb' : 'kg'}</Text>
            </View>
            <TouchableOpacity style={styles.selectButton} onPress={() => setShowWeightModal(true)}>
        ...
        <View style={styles.footer}>
          <TouchableOpacity style={[styles.saveButton, isSaving && styles.disabledButton]} onPress={handleSave} disabled={isSaving}>
            {isSaving ? <ActivityIndicator color="#2C3E50" /> : <Text style={styles.saveButtonText}>{i18n.t("ui.saglabat_svaru")}</Text>}
          </TouchableOpacity>
        </View>
